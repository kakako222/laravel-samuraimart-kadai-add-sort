...
// MAMPを利用しているMacユーザーの方は、''ではなく'root'を代入してください
 ...
 if (isset($_POST['submit'])) {
     try {
         ...
         //動的に変わる値をブレースホルダに置き換えたUPDATE文をあらかじめ用意する
         $sql = '
              UPDATE users
              SET name = :name,
              furigana = :furigana,
 ...
 if(isset($_GET['id'])){
    try{
        $pdo = new PDO ($dsn, $user, $password);
        //idカラムの値をブレースホル(:id)に置き換えたSQL文をあらかじめ用意する
        ...
        //SQL文を実行結果を配列で取得する
        //補足：１つのレコード（横１列のデータ）のみを取得したい場合、fetch()メソッドを使えば絡むキーになった１次元配列を取得できる
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        //idパラメータの値と同じidのデータが存在しない場合ではエラーメッセージを表示して処理を終了します
    ...
    //idパラメーターの値が存在しないときはエラ〜メッセージを表示して処理を停止
